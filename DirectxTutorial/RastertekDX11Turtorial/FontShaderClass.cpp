...
{
	bool result;
	//Initialize loads the new font vertex shaderand pixel shader HLSL files.
...
//Shutdown calls ShutdownShader which releases the font shader related pointersand data.
...
//Render will set the shader parametersand then draw the buffers using the font shader.Notice the is the same as TextureShaderClass except that it takes in the new pixelColor parameter.
...
//The InitializeShader function loads the new HLSL font vertexand pixel shaders as well as the pointers that interface with the shader.
	...
	D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, D3DXVECTOR4 pixelColor)
{
	...
	//Here is where we set the pixel color before rendering.We lock the pixel constant bufferand then set the pixel color inside itand unlock it again.We set the constant buffer position in the pixel shaderand it is ready for use.
